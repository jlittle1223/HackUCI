...
class DirectedEdge(Edge):
    ...
    def __str__(self) -> str:
        return "DirectedEdge({num}, {source} -> {destination})".format(
            ...
            destination = self.vertices[1])
        
        
        
class DirectedDisjunctionEdge(DirectedEdge):
    def __init__(self, num:int, source:'Vertex', destination:'Vertex', disjunctionID):
        ''' initialize the edge '''

        DirectedEdge.__init__(self, num, source, destination)
        
        self.disjunctionID = disjunctionID
